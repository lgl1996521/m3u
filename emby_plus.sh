...
if [ $1 ]; then
	...
	docker0=$(ifconfig docker0 | grep inet |awk '{print $2}'|tr -d "addr:")
	if [ $2 ]; then
	...
	wget -O /tmp/Emby.Server.Implementations.dll http://docker.xiaoya.pro/Emby.Server.Implementations.dll
	case $cpu_arch in
		"x86_64" | *"amd64"*)
